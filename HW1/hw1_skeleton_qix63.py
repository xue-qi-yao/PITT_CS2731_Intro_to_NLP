...
def create_term_document_matrix(line_tuples, document_names, vocab):
    ...
    doc_dict = {doc: i for i, doc in enumerate(document_names)}

    m, n = len(vocab), len(document_names)
    td_matrix = np.zeros((m, n), dtype=np.int32)

    for doc_name, tokens in line_tuples:
      c_idx = doc_dict[doc_name]
      for word in tokens:
          r_idx = vocab_dict[word]
          td_matrix[r_idx, c_idx] += 1
...
def create_tf_idf_matrix(term_document_matrix):
    ...
    """
    term_document_matrix = term_document_matrix.astype(np.float64)
    with np.errstate(divide='ignore', invalid='ignore'): 
      tf = np.where(term_document_matrix > 0, 1 + np.log10(term_document_matrix), 0)
    document_frequencies = np.count_nonzero(term_document_matrix, axis=1)
    num_documents = term_document_matrix.shape[1]
    idf = np.log((num_documents + 1) / (document_frequencies + 1))
    tf_idf_matrix = tf * idf[:, np.newaxis] 
