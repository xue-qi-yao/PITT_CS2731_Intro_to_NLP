...
class LogisticRegression():
    ...
    def train(self):
        ...
        for i in range(self.num_iter):
            ...
            # self._loss()
            self.test()
            print(f"round {i} | accuracy rate: {self.acc}")
            result_filter = (self.pred != y).squeeze()
            filtted_y = y[:, result_filter]
            filtted_x = x[:, result_filter]
            filtted_pred = self.pred[:, result_filter]
            self.a += self.lr * np.sum(filtted_x * (filtted_y - filtted_pred) / 2, axis=1, keepdims=True).T
            self.b += self.lr * np.sum((filtted_y - filtted_pred) / 2, axis=1)
